...
export class TournamentService {
	...
	getTournaments(): Observable<Tournament[]> {
		return this.http.get<Tournament[]>(endpoint,{headers:httpOptions.headers});
	}
	...
	find(id: string): Observable<Tournament> {
		return this.http.get<Tournament>(endpoint + "tournament/" + id, {headers:httpOptions.headers});
	}

	findByName(name: string): Observable<Tournament[]> {
		return this.http.get<Tournament[]>(`${endpoint}?name=${name}`,{headers:httpOptions.headers});
	}

	getParticipants(id: string): Observable<any[]> {
		return this.http.get<any[]>(`${endpoint}participants?id=${id}`,{headers:httpOptions.headers});
	}

	delete(id: string, ownerId: number) {
		return this.http.delete(
			endpoint + `user/tournaments?id=${id}&owner=${ownerId}`,{headers:httpOptions.headers}
		);
	}
	...
	updateState(id: number) {
		return this.http.put(endpoint + "tournamentState/" + id, "", httpOptions);
	}
	...
	getBracket(id: string) {
		return this.http.get<Bracket>(`${endpoint}bracket?id=${id}`, {headers:httpOptions.headers});
	}

	updateSchedule(bracket: any) {
		return this.http.put<any>(`${endpoint}bracket/update`, bracket, httpOptions);
	}
